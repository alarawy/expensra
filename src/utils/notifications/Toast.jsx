...
export const showToast = (
  ...
  return toast.custom((tObj) => (
    <div
      className={`flex items-center gap-2 rounded-lg px-4 py-3 text-sm shadow-md
        ${variant === "success" && "bg-green-600 text-white"}
        ${variant === "error" && "bg-red-600 text-white"}
        ${variant === "loading" && "bg-gray-700 text-white"}
        ${tObj.visible ? "animate-in fade-in" : "animate-out fade-out"}
      `}
    >
      <span className="text-lg">{icons[variant]}</span>
      <span>{text}</span>
